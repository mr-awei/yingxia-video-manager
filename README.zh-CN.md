...
- **随机截帧 + 质量过滤**：缺封面时 ffmpeg 从 12–22 个候选帧里自动跳过黑屏、白屏、模糊、单调画面，每次「重新截图」都能拿到不同且清晰的画面。
- **安装器第一步选语言**：NSIS 安装器首屏弹出语言选择（简体中文 / English），应用首次打开即为选定语言；卸载器也沿用该语言，设置页可随时切换。
- **英文用户须知去中国法律化**：用户须知弹窗按当前 locale 分支渲染——中文用户保留中国法律节选；英文用户展示通用免责声明，不再引用《刑法》《治安管理处罚法》《网络安全法》《未成年人保护法》《民法典》等具体中国法律条文。
