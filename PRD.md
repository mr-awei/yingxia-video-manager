...
> 所有功能模块均经代码核实（截至 v2.5.1），标注【当前存在】的模块为 v2.5.1 中仍在运行的功能。
...
| 国际化与合规 | v2.5.1 | v2 | NSIS 安装器第一步语言选择并写入注册表；主进程首次启动读取安装器语言；英文用户须知移除中国法律法规引用；卸载器沿用安装语言 |
...
:*文档结束。本 PRD 覆盖截至 v2.5.1 的全部现存功能；新增需求请追加至第 10 节 Roadmap 并评审。*
